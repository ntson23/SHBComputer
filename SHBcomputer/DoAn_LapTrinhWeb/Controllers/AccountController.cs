...
namespace DoAn_LapTrinhWeb.Controllers
{
    public class AccountController : Controller
    {
        ...
        public ActionResult Register([Bind(Exclude = "status,Requestcode")] Account accounts, Register model)
        {
            ...
            {
                accounts.role_id = Const.ROLE_MEMBER_CODE; //admin quyền là 4: thành viên quyền là 1, biên tập viên là 5, người kiểm duyệt là 6             
        ...
        public ActionResult ResetPassword(ResetPassword model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null && user.expired_at > DateTime.Now)
                {
                    ...
                    //sau khi đổi mật khẩu thành công khi quay lại link cũ thì sẽ không đổi được mật khẩu nữa 
                    user.Requestcode = "";
