...
namespace DoAn_LapTrinhWeb.Controllers
{
    public class PaymentMethodsController : Controller
    {
        ...
        private Payment PayPalCreatePayment(APIContext apiContext, string redirectUrl)
        {
            ...
            {
                description = "SHBComputer - Thanh toan don hang " + order.order_id,
        ...
        public ActionResult vnpay(object sender, EventArgs e)
        {
            ...
            vnpay.AddRequestData("vnp_OrderInfo", "SHBComputer - Thanh toan don hang: #" + order.OrderId);
            ...
            if (Request.QueryString.Count > 0)
            {
                ...
                if (checkSignature)//kiểm tra chữ ký
                {
                    if (vnp_ResponseCode == "00" && vnp_TransactionStatus == "00")
                    {
                        ...
                        SendMailAdmin(SubjectMail, OrderPayment, OrderDelivery, DeliveryId, SubOrderTotal, Discount_Price, UserEmail, UserName, UserPhoneNumber, UserAddress, OrderId, OrderStatus, OrderTotal, ProductOrder);
                        // Gửi mail cho khách hàng khi thanh toán thành công
                        SendVerificationLinkEmail(DeliveryId, SubjectMail, ButtonConfirmlink, ButtonConfirm, OrderPayment, OrderDelivery, SubOrderTotal, Discount_Price, UserEmail, UserName, UserPhoneNumber, UserAddress, OrderId, OrderStatus, OrderTotal, ProductOrder);
                    }
                    else
                    {
        ...
        //Gửi mail thông báo đơn hàng cho Admin
        [NonAction]
        public void SendMailAdmin(string SubjectMail, string OrderPayment, string OrderDelivery, string DeliveryId, string SubOrderTotal, string Discount_Price, string UserEmail, string UserNameOrder, string UserPhoneNumber, string UserAddress, string OrderId, string OrderStatus, string OrderTotal, string ProductOrder)
        {
            ...
            var toEmail = new MailAddress(AccountEmail.UserEmail);
            ...
            body = body.Replace("{{BodyContent}}", "Có đơn hàng <span style='color: rgb(1, 181, 187); font-weight: 500;'>#" + OrderId + "</span> đã được đặt với hình thức thanh toán là <span>" + OrderPayment);
            body = body.Replace("{{OrderStatus}}", OrderStatus);
            body = body.Replace("{{ButtonConfirm}}", "");
            body = body.Replace("{{ButtonConfirmLink}}", "");
            // Thêm style ẩn đi các phần tử có tên là ButtonConfirm và ButtonConfirmLink
            body = body.Replace("<td>{{ButtonConfirm}}</td>", "<td style='display:none;'>{{ButtonConfirm}}</td>");
            body = body.Replace("<td>{{ButtonConfirmLink}}</td>", "<td style='display:none;'>{{ButtonConfirmLink}}</td>");
            body = body.Replace("{{UserEmail}}", UserEmail);
            body = body.Replace("{{DiscountPrice}}", Discount_Price);
            body = body.Replace("{{UserName}}", AccountEmail.UserName);
            body = body.Replace("{{UserNameOrder}}", UserNameOrder);
        ...
        public void SendVerificationLinkEmail(string DeliveryId, string SubjectMail, string ButtonConfirmlink, string ButtonConfirm, string OrderPayment, string OrderDelivery, string SubOrderTotal, string Discount_Price, string UserEmail, string UserName, string UserPhoneNumber, string UserAddress, string OrderId, string OrderStatus, string OrderTotal, string ProductOrder)
        {
            ...
            body = body.Replace("{{UserNameOrder}}", UserName);
            body = body.Replace("{{UserAddress}}", UserAddress);
            body = body.Replace("{{UserPhoneNumber}}", UserPhoneNumber);
            body = body.Replace("{{SubOrderTotal}}", SubOrderTotal);
            body = body.Replace("{{OrderTotal}}", OrderTotal);
