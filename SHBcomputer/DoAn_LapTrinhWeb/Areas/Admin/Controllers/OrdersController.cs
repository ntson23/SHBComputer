    ...
    public class OrdersController : BaseController
    {
        ...
        public ActionResult OrderDetails(int? id)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    ...
                    {
                        Notification.set_flash("Không tồn tại đơn hàng: " + "SHB" + id + ")", "warning");
            ...
            if (User.Identity.IsAuthenticated)
            {
                if (User.Identity.Permiss_Update() == true || User.Identity.Permiss_Modify() == true)
                {
                    ...
                    if (order != null)
                    {
                        ...
                        db.Entry(order).State = EntityState.Modified;
                        db.SaveChanges();
                        Notification.set_flash("Đã chuyển trạng thái đơn hàng: " + "SHB" + id + " sang chờ xử lý!", "success");
                    }
                    else
                    {
        ...
        string OrderPhone, string OrderName, string OrderAddress, string OrderDelivery, string DeliveryId, string emailFor)
        {
            ...
            if (emailFor == "ChangeProcessing")
            {
                ...
                body = body.Replace("{{UserNameOrder}}", OrderName);
            ...
            else if (emailFor == "ChangeComplete")
            {
                ...
                body = body.Replace("{{OrderId}}", OrderID);
                body = body.Replace("{{BodyContent}}", "Đơn hàng <span style='color: rgb(1, 181, 187); font-weight: 500;'>#" + OrderID + "</span> đã được giao thành công. Cảm ơn bạn đã tin tưởng SHBComputer, hi vọng trong tương lai bạn sẽ tiếp tục sử dụng dịch vụ của Chúng tôi.");
                body = body.Replace("{{ButtonConfirm}}", "Quản lý đơn hàng");
                body = body.Replace("{{ButtonConfirmLink}}", ButtonConfirmlink);
                body = body.Replace("{{OrderStatus}}", OrderStatus);
                body = body.Replace("{{UserEmail}}", emailID);
            ...
            else if (emailFor == "CancleOrders")
            {
                ...
                body = body.Replace("{{OrderId}}", OrderID);
                body = body.Replace("{{BodyContent}}", "Đơn hàng <span style='color: rgb(1, 181, 187); font-weight: 500;'>#" + OrderID + "</span> đã được hủy theo yêu cầu của bạn. SHBComputer bắt đầu tiến hành thủ tục hoàn tiền và sẽ thông tin đến bạn khi đã hoàn tất.");
                body = body.Replace("{{ButtonConfirm}}", "Quản lý đơn hàng");
                body = body.Replace("{{ButtonConfirmLink}}", ButtonConfirmlink);
                body = body.Replace("{{OrderStatus}}", OrderStatus);
                body = body.Replace("{{UserEmail}}", emailID);
        ...
        public void ExportDataToExcel(DateTime picker1, DateTime picker2, string order__status, int payment__method)
        {
            ...
            using (ExcelEngine excelEngine = new ExcelEngine())
            {
                ...
                workbook.SaveAs("NVT_Report_" + picker1.ToString("dd-MM-yyyy") + "_" + picker2.ToString("dd-MM-yyyy") + "_" + file + ".xlsx", ExcelSaveType.SaveAsXLS, HttpContext.ApplicationInstance.Response, ExcelDownloadType.Open);
            }
        }
